...
namespace StringCompare
{
    public partial class Form1 : Form
    {
        ...
        private void parseHTML(string inHtml,ref string outHtml)
        {
            string rc = inHtml.Replace("<", " <");
            outHtml = rc.Replace(">", "> ");
            inHtml = outHtml.Replace("=", "= ");
            outHtml = inHtml.Replace("\"", "\" ");
            inHtml = outHtml.Replace(",", " ,");
            outHtml = inHtml.Replace("'", " '");
            inHtml = outHtml.Replace("><", "> <");
            outHtml = inHtml.Replace(";", "; ");
            inHtml = outHtml.Replace("\r", "");
            outHtml = inHtml.Replace("\n", "");
            while (outHtml.Length != inHtml.Length)
            {
                  inHtml=outHtml;
                outHtml = inHtml.Replace("  ", " ");
            }
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            if (!Render)
            {
                ...
                if (this.checkBoxhtml.Checked)
                    parseHTML(this.textBox1.Text, ref text);
                else
        ...
        {
            this.textBox1.Text = this.textBox1.Text.Replace(this.textBoxFind.Text, this.textBoxReplace.Text);

            if (!this.checkBoxCleanAll.Checked)
                return;
        ...
        {
            string[] data = this.textBox1.Text.Split(new char[]{' '});
            int datlen=data.Length-1;
            for (int i = 0; i < datlen; i++)
            {
                if (data[i].Length < 4)
                {
                    data[i + 1] = data[i] + "_" + data[i + 1];
                    data[i] = "";
                }
            }
            this.textBox1.Text = String.Join(" ", data);
        }
    }
}
